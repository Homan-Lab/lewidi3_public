...
import numpy as np
import pandas as pd
from utils.utils import load_object, create_annotator_metadata_mapping, get_annotator_metadata
...
def save_predictions_tsv(data_to_write, output_file):
    """
    Save predictions in the desired TSV format:
    - First column: unique identifier of the item (original JSON key)
    - Second column: list containing predictions for each annotator
    """
    # Prepare data in the correct format
    formatted_rows = []
    
    for item in data_to_write:
        unique_id = item['message_id']  # This should be the original JSON key
        predictions = item['labels']    # This is the list of predictions
        
        # Format as tab-separated values
        formatted_rows.append(f"{unique_id}\t{predictions}")
...
def save_pred_ann(data_to_write, op):
    formatted_rows = []
    
    for item in data_to_write:
        unique_id = item['message_id']
        annotator_labels = item.get('annotator_labels', [])  # individual annotator predictions (class indices 0-10)
        
        # Convert class indices to Likert scale (-5 to +5) by subtracting 5
        #likert_labels = [label - 5 for label in annotator_labels]  #use this for paraphrase --> for scaling
        #likert_labels = [label + 1 for label in annotator_labels]  #use this for csc
        #formatted_rows.append(f"{unique_id}\t{likert_labels}")  #uncomment this to run par or csc --> for scaling
        formatted_rows.append(f"{unique_id}\t{annotator_labels}")   # use this for mp

    with open(op, 'w') as f:
        for row in formatted_rows:
            f.write(row + '\n')

def parse_one_hot_string(s):
...
def main():
    ...
    if A.shape[1] > 1:
        annotator_ids_from_A = A[:, 0].astype(int)
        annotator_metadata_from_A = A[:, 1:]
        print("A matrix structure: annotator_ids + metadata embeddings")
        print("  annotator_ids shape:", annotator_ids_from_A.shape)
        print("  annotator_metadata shape:", annotator_metadata_from_A.shape)
    else:
        annotator_ids_from_A = A[:, 0].astype(int)
        annotator_metadata_from_A = None
    ...
    # Create metadata mapping if available
    metadata_mapping = None
    if annotator_metadata_from_A is not None:
        # Create mapping from annotator ID to metadata embedding
        unique_annotator_ids = np.unique(annotator_ids_from_A)
        metadata_mapping = {}
        
        for ann_id in unique_annotator_ids:
            # Find the first occurrence of this annotator ID and get their metadata
            ann_indices = np.where(annotator_ids_from_A == ann_id)[0]
            if len(ann_indices) > 0:
                metadata_mapping[ann_id] = annotator_metadata_from_A[ann_indices[0]]
        
        print("Created metadata mapping for {} unique annotators".format(len(metadata_mapping)))
        
        # Create matrix of all unique annotator metadata for ensemble prediction
        unique_metadata_list = list(metadata_mapping.values())
        all_annotator_metadata = np.array(unique_metadata_list)
        print("All annotator metadata shape for ensemble:", all_annotator_metadata.shape)
    else:
        all_annotator_metadata = None
        print("No metadata available for ensemble prediction")

    for unique_dataitem in tqdm(unique_dataitems):
        item_index = raw_item_annotator_label.loc[raw_item_annotator_label['item'] == unique_dataitem].index[0]
        ...
        original_comment_id = unique_dataitem
        item_annotations = raw_item_annotator_label.loc[raw_item_annotator_label['item'] == unique_dataitem]
        annotator_ids = item_annotations['annotator'].tolist()
        annotator_labels_raw = item_annotations['label'].tolist()
        annotator_labels_raw = item_annotations['label'].tolist()
        annotator_labels = []
        for l in annotator_labels_raw:
            if isinstance(l, str):
                one_hot = parse_one_hot_string(l)
            else:
                one_hot = np.array(l)
            annotator_labels.append(int(np.argmax(one_hot)))
...
################################################################################
        ...
        with tf.device(gpu_tag):
            ...
            i_s = I[s_ptr,:]
            a_s = A[s_ptr, 1:]  # Use only metadata part (skip annotator ID column)

            py, _ = model.decode_y_ensemble(x_s, all_annotator_metadata=all_annotator_metadata)
            
            # Use softmax probabilities directly instead of converting to hard labels
            # py contains softmax probabilities [batch_size, num_classes]
            predicted_label_dist = py.numpy()[0].tolist()  # Get probabilities for first (and only) item in batch
            
            # Debug: Verify we have softmax probabilities
            prob_sum = sum(predicted_label_dist)
            if abs(prob_sum - 1.0) > 1e-6:
                print(f"WARNING: Probabilities don't sum to 1.0: {prob_sum}")
            
            # Debug: Show first few predictions
            if len(predicitions) < 3:
                print(f"Item {unique_dataitem} - Softmax probabilities: {[f'{p:.4f}' for p in predicted_label_dist[:5]]}")
                print(f"  Sum of probabilities: {prob_sum:.6f}")
                print(f"  Max probability: {max(predicted_label_dist):.4f} at class {np.argmax(predicted_label_dist)}")


            annotator_hard_predictions = []
            for annotator_id in annotator_ids:
                # Get metadata embedding for this annotator
                if metadata_mapping is not None and annotator_id in metadata_mapping:
                    meta_embedding = metadata_mapping[annotator_id]
                    meta_embedding = np.expand_dims(meta_embedding, axis=0)
                    meta_embedding = tf.cast(meta_embedding, dtype=tf.float32)
                else:
                    # Fallback to dummy metadata if mapping not available
                    meta_embedding = tf.zeros([1, a_s.shape[0]], dtype=tf.float32)
                
                z = model.encode(x_s, meta_embedding)
                yi_prob, yi_logits = model.decode_yi(z)
                annotator_pred_hard = int(tf.argmax(yi_prob, axis=1).numpy()[0])
                
                annotator_hard_predictions.append(annotator_pred_hard)

            if len(predicted_label_dist) == len(empirical_label):
                predicitions.append(predicted_label_dist)
                row_to_write = {}
                row_to_write['message_id'] = original_comment_id
                row_to_write['labels'] = predicted_label_dist
                row_to_write['annotator_ids'] = annotator_ids  # annotator IDs
                row_to_write['annotator_labels'] = annotator_hard_predictions
    ...
    output_file = data_dir + f"/op_predictions.tsv"
    save_predictions_tsv(data_to_write, output_file)
    op = data_dir + f"/op_ann.tsv"
    save_pred_ann(data_to_write, op)
