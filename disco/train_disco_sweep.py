...
def calc_stats(model, Xi_, Yi_, Ya_, Y_, A_, I_, batch_size, agg_type="mode",
               n_subset=1000, eval_aggreg=True, all_annotator_metadata=None, use_evaluation_loss=False, alpha=0.5, objective_type="combined"):
    """
    ...
    while ptr_s < len(ptrs):
        ...
        i_s = I[ptr_indx, :]
        a_s = A[ptr_indx, 1:]  # Use only metadata part (skip annotator ID column)
        ...
        a_s = tf.cast(a_s, dtype=tf.float32)  # Ensure metadata is float32
        ...
        # L += tf.reduce_sum(Ly)
        Ns = y_s.shape[0]
        L_t, Ly_t, KLi_t, KLa_t = model.calc_loss(y_s, yi_s, ya_s, pY, pYi, pYa, use_evaluation_loss=use_evaluation_loss, alpha=alpha, objective_type=objective_type)  # compute cost (loss over entire design matrices)
        L += L_t * Ns
        KLi += KLi_t * Ns
        KLa += KLa_t * Ns
        ...
        if eval_aggreg is True:
            for s in range(xi_s.shape[0]):
                ...
                py, _ = model.decode_y_ensemble(xs, all_annotator_metadata=all_annotator_metadata)
    ...
    results = classification_report(y_test, y_pred, digits=3, output_dict=True, zero_division=0)
...
def train_disco(data, simulation_params, disco_model_params, params, use_evaluation_loss=False, alpha=0.5, objective_type="combined"):
    # Get metadata dimension from data
    meta_dim = data.get("meta_dim", 768)  # Default to 768 if not specified
    
    # Debug: print model parameters
    print(f"Model parameters:")
    print(f"  drop_p: {disco_model_params['drop_p']}")
    print(f"  lat_dim: {disco_model_params['lat_dim']}")
    print(f"  meta_dim: {meta_dim}")
    print(f"  use_evaluation_loss: {use_evaluation_loss}")
    if use_evaluation_loss:
        print(f"  objective_type: {objective_type}")
        if objective_type == "combined":
            print(f"  alpha: {alpha}")
        elif objective_type == "alternating":
            print(f"  alternating every 100 steps")
    
    model = DISCO(xi_dim=data["n_xi"], yi_dim=data["yi_dim"], ya_dim=data["ya_dim"], y_dim=data["y_dim"],
                  a_dim=data["n_a"],  # This now represents metadata embedding dimension
                  ...
                  gamma_a=disco_model_params["gamma_a"], meta_dim=meta_dim)
    ...
    acc, L, KLi, KLa, agg_acc, f1_macro, f1_micro, f1_weighted, precision_macro, precision_weighted, recall_macro, recall_weighted,train_agg_KL = calc_stats(model, data["Xi"], data["Yi"], data["Ya"], data["Y"], data["A"], data["I"],
                                           simulation_params["batch_size"], all_annotator_metadata=data.get("annotator_metadata"), use_evaluation_loss=use_evaluation_loss, alpha=alpha, objective_type=objective_type)
    if data["dev_Y"] is not None:
        ...
        dev_acc, dev_L, dev_KLi, dev_KLa, dev_agg_acc, dev_f1_macro, dev_f1_micro, dev_f1_weighted, dev_precision_macro, dev_precision_weighted, dev_recall_macro, dev_recall_weighted,dev_agg_KL = calc_stats(model, data["dev_Xi"], data["dev_Yi"],
                                                                   ...
                                                                   data["dev_I"], simulation_params["batch_size"], all_annotator_metadata=data.get("dev_annotator_metadata"), use_evaluation_loss=use_evaluation_loss, alpha=alpha, objective_type=objective_type)
    ...
    # Track global step for alternating objectives
    global_step = 0
    ...
    for e in range(simulation_params["n_epoch"]):
        ...
        while ptr_s < len(ptrs):
            ...
            # sample without replacement the label distribution data
            i_s = data["I"][ptr_indx, :]
            a_s = data["A"][ptr_indx, 1:]  # Use only metadata part (skip annotator ID column)
            ...
            L_t = model.update(xi_s, a_s, yi_s, ya_s, y_s, disco_model_params["update_radius"], use_evaluation_loss=use_evaluation_loss, alpha=alpha, objective_type=objective_type, step=global_step)
            L = (L_t * y_s.shape[0]) + L
            Ns += y_s.shape[0]
            global_step += 1
            
            # Print current objective for alternating training
            if objective_type == "alternating" and global_step % 50 == 0:
                current_obj = "soft" if (global_step % 200) < 100 else "perspectivist"
                print(f"\r{global_step}: {current_obj} objective, L = {(L / Ns):.4f}", end="")
            else:
        ...
        if e % simulation_params["eval_every"] == 0:
            acc, L, KLi, KLa, agg_acc, f1_macro, f1_micro, f1_weighted, precision_macro, precision_weighted, recall_macro, recall_weighted, train_agg_KL = calc_stats(model, data["Xi"], data["Yi"], data["Ya"], data["Y"], data["A"],
                                                   data["I"], simulation_params["batch_size"], all_annotator_metadata=data.get("annotator_metadata"), use_evaluation_loss=use_evaluation_loss, alpha=alpha, objective_type=objective_type)
            if data["dev_Y"] is not None:
                dev_acc, dev_L, dev_KLi, dev_KLa, dev_agg_acc, dev_f1_macro, dev_f1_micro, dev_f1_weighted, dev_precision_macro, dev_precision_weighted, dev_recall_macro, dev_recall_weighted, dev_agg_KL = calc_stats(model, data["dev_Xi"], data["dev_Yi"],
                                                                           data["dev_Ya"], data["dev_Y"], data["dev_A"],
                                                                           data["dev_I"],
                                                                           simulation_params["batch_size"], all_annotator_metadata=data.get("dev_annotator_metadata"), use_evaluation_loss=use_evaluation_loss, alpha=alpha, objective_type=objective_type)
...
def read_wandb_sweep_id(sweep_id, params, simulation_params, gpu_tag, run_count, disco_model_params, use_evaluation_loss=False, alpha=0.5, objective_type="combined"):
        ...
        with wandb.init(config=config):
            ...
            with tf.device(gpu_tag):
                train_disco(data, simulation_params, disco_model_params, params, 
                           use_evaluation_loss=use_evaluation_loss, 
                           alpha=alpha,
...
def main():
    parser = argparse.ArgumentParser(description='Train DISCO model with different objective types')
    parser.add_argument("--config", help="Config file", required=True)
    parser.add_argument("--sweep_id", help="Sweep ID from WANDB", required=True)
    parser.add_argument("--gpu_id", help="GPU id", default=-1)
    parser.add_argument("--run_count", help="Run Count", default=10)
    parser.add_argument("--objective_type", choices=["original", "soft", "perspectivist", "combined", "alternating"], 
                       default="original", help="Type of objective to use")
    parser.add_argument("--alpha", type=float, default=0.5,
                       help="Weight for soft label vs perspectivist (only for combined objective)")
    ...
    # Get parameters
    params, simulation_params, disco_model_params = get_params(args.config)
    
    print("=" * 70)
    print("DISCO TRAINING WITH MULTI-OBJECTIVE SUPPORT")
    print("=" * 70)
    print(f"Config file: {args.config}")
    print(f"Objective type: {args.objective_type}")
    
    if args.objective_type == "original":
        print("Training with original loss functions:")
        print("  - Categorical NLL for ground truth")
        print("  - KL divergence for item distributions")
        print("  - KL divergence for annotator distributions")
        use_evaluation_loss = False
    elif args.objective_type == "soft":
        print("Training with soft label optimization:")
        print("  - Wasserstein distance for distribution similarity")
        print("  - Optimized for soft label evaluation")
        use_evaluation_loss = True
    elif args.objective_type == "perspectivist":
        print("Training with perspectivist optimization:")
        print("  - Mean absolute distance for expected value accuracy")
        print("  - Optimized for perspectivist evaluation")
        use_evaluation_loss = True
    elif args.objective_type == "combined":
        print("Training with combined objectives:")
        print(f"  - Soft label weight: {args.alpha}")
        print(f"  - Perspectivist weight: {1 - args.alpha}")
        use_evaluation_loss = True
    elif args.objective_type == "alternating":
        print("Training with alternating objectives:")
        print("  - Alternates between soft and perspectivist every 100 steps")
        print("  - Balances both evaluation metrics")
        use_evaluation_loss = True
    
    print(f"Regularization: L1={disco_model_params.get('l1_norm', 0)}, L2={disco_model_params.get('l2_norm', 0)}")
    print("=" * 70)
    
    # Train with specified objective
    def train_with_objective(config=None):
        with wandb.init(config=config):
            disco_model_params = wandb.config
            with tf.device(gpu_tag):
                # Load data
    ...
    # Run training
    read_wandb_sweep_id(args.sweep_id, params, simulation_params, gpu_tag, 
                       int(args.run_count), disco_model_params, use_evaluation_loss=use_evaluation_loss, alpha=args.alpha, objective_type=args.objective_type)
