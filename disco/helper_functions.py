...
def sentence_embedding(data_items,data_index):
    ...
    # embedder = SentenceTransformer('intfparaphrase-MiniLM-L6-v2loat/multilingual-e5-large')
...
def metadata_to_sentence(metadata):
    """
    hndles metadata to natural language
    """
    
"""
    # use this for mp
    age = metadata.get('Age', 'unknown age')
    gender = metadata.get('Gender', 'unknown gender')
    nationality = metadata.get('Nationality', 'unknown nationality')
    country_birth = metadata.get('Country of birth', 'unknown country')
    country_residence = metadata.get('Country of residence', 'unknown country')
    student_status = metadata.get('Student status', 'unknown student status')
    employment_status = metadata.get('Employment status', 'unknown employment status')
    ethnicity = metadata.get('Ethnicity simplified', 'unknown ethnicity')
        
    # Create descriptive sentence for MP metadata
    sentence_parts = []
    sentence_parts.append(f"The annotator is a {age} year old {gender.lower()}")
        
    if nationality != 'unknown nationality':
        sentence_parts.append(f"from {nationality}")
        
    if country_birth != 'unknown country' and country_birth != 'DATA_EXPIRED':
        sentence_parts.append(f"born in {country_birth}")
        
    if country_residence != 'unknown country' and country_residence != 'DATA_EXPIRED':
        sentence_parts.append(f"residing in {country_residence}")
        
    if student_status != 'unknown student status' and student_status != 'DATA_EXPIRED':
        sentence_parts.append(f"student status: {student_status}")
        
    if employment_status != 'unknown employment status' and employment_status != 'DATA_EXPIRED':
        sentence_parts.append(f"employment status: {employment_status}")
        
    if ethnicity != 'unknown ethnicity' and ethnicity != 'DATA_EXPIRED':
        sentence_parts.append(f"ethnicity: {ethnicity}")
    ...
    if age != 'unknown age' and age != 'DATA_EXPIRED':
        sentence_parts.append(f" The annotator is {age} year old")
        
    if gender != 'unknown gender' and gender != 'DATA_EXPIRED':
        sentence_parts.append(f"{gender}")
    ...
    #use this for paraphrase

    return (
            f"The annotator is a {metadata['Age']} year old "
            f"{metadata['Nationality']} {metadata['Gender'].lower()} "
            f"{metadata['Education']}."
        )
